...
exports.MIN_SEEN_COUNT_FOR_BLOCK = void 0;
exports.computeMemoryMatchScore = computeMemoryMatchScore;
exports.buildMemorySignal = buildMemorySignal;
exports.findBestMemoryMatch = findBestMemoryMatch;
function overlapCount(a, b) {
    const setB = new Set(b);
    return a.filter((x) => setB.has(x)).length;
}
function computeMemoryMatchScore(candidate, profile) {
    ...
    return (piiOverlap * 10 +
        tagOverlap * 6 +
        comboOverlap * 20 +
        Math.min(profile.seenCount, 5) * 4 +
        profile.riskWeight * 20);
}
exports.MIN_SEEN_COUNT_FOR_BLOCK = 2;
function shouldBlockForCandidate(candidate, matchScore, profileSeenCount) {
    if (profileSeenCount < exports.MIN_SEEN_COUNT_FOR_BLOCK)
        return false;
    if (matchScore < 70)
        return false;
    ...
    if (publicUpload && thirdParty)
        return true;
    if (idCard && hasAnyPii)
        return true;
    if (document && hasAnyPii)
        return true;
    if (face && nameLike && publicUpload)
        return true;
    if (face && affiliation && tagSet.has('social_media'))
        return true;
    if (gps && publicUpload)
        return true;
    return combos.some((c) => c.includes('public_upload') &&
        (c.includes('document') || c.includes('third_party') || c.includes('id_card')));
}
function buildMemorySignal(matchScore, candidate, settings, profileSeenCount = 0) {
    ...
    const shouldBlock = settings.useForBlocking && shouldBlockForCandidate(candidate, matchScore, profileSeenCount);
    return {
        matched: true,
        memoryMatchScore: matchScore,
        piiBoost,
...
function findBestMemoryMatch(candidate, profiles, settings) {
    let bestScore = 0;
    let bestSignal = {
        matched: false,
        memoryMatchScore: 0,
...
//# sourceMappingURL=match.util.js.map